...
class goblin(Personaje):
    ...
    def arrojar_daga(self,humano):
        if self.energia < 15 and self.energia >= 10:
    ...
    def golpe_critico(self,humano):
        if self.energia < 40 and self.energia >= 10:
            print(f'{self.nombre} Intenta hacer un golpe crítico pero está muy cansado')
            ataque_random = random.randint(1,2)
            if ataque_random == 1:
                self.puñalada(humano)
...
class humano(Personaje):
    ...
    def golpe_poderoso(self, goblin):
        if self.energia < 15 and self.energia > 10:
    ...
    def torbellino_espadas(self,goblin):
        if self.energia < 40 and self.energia >= 10:
...
class_dict = {'goblin' : goblin, 'humano' : humano}
 
jugador_1 = class_dict[personaje](nombre, reino)

class_dict.pop(personaje)
randomvalues = random.choice(list(class_dict))
#desde aca tratando de que seleccione el otro personaje para jugados 2
